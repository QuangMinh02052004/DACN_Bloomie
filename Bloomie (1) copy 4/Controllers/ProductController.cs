...
namespace Bloomie.Controllers
{
    public class ProductController : Controller
    {
        ...
        // ====== CHỨC NĂNG TÌM KIẾM HÌNH ẢNH MỚI ======

        /// <summary>
        /// Model kết quả nhận diện hoa
        /// </summary>
        public class FlowerRecognitionResult
        {
        ...
        private async Task<List<Product>> FindProductsByFlowerName(string flowerName)
        {
            try
            {
                ...
                var productsByName = await _context.Products
                    .Where(p => p.IsActive &&
                           (p.Name.ToLower().Contains(flowerName.ToLower()) ||
                            p.Description.ToLower().Contains(flowerName.ToLower())))
                    .ToListAsync();

                // Tìm trong loại hoa (FlowerTypes)
                var productsByFlowerType = await _context.Products
                    .Where(p => p.IsActive)
                    .Include(p => p.FlowerTypeProducts)
                    .ThenInclude(ftp => ftp.FlowerType)
                    .Where(p => p.FlowerTypeProducts.Any(ftp =>
                        ftp.FlowerType.Name.ToLower().Contains(flowerName.ToLower())))
                    .ToListAsync();

                // Kết hợp kết quả và loại bỏ trùng lặp
                var allProducts = productsByName.Union(productsByFlowerType).Distinct().ToList();
        ...
        /// <summary>
        /// Hiển thị kết quả tìm kiếm bằng hình ảnh
        /// </summary>
        [HttpGet]
        public async Task<IActionResult> ImageSearchResults(string flowerName, double probability = 0)
        {
            if (string.IsNullOrEmpty(flowerName))
            {
                TempData["error"] = "Không tìm thấy thông tin nhận diện.";
                return RedirectToAction("Index");
            }

            // Tìm sản phẩm phù hợp
            var matchedProducts = await FindProductsByFlowerName(flowerName);
            ...
            // Tạo danh sách sản phẩm với rating
            var productsWithRating = new List<ProductViewModel.ProductWithRating>();
            foreach (var product in matchedProducts)
            {
                var averageRating = (await _context.Ratings
            ...
            ViewBag.Categories = parentCategories ?? new List<Category>();
            ViewBag.RecognizedFlowerName = flowerName;
            ViewBag.Probability = probability;
            ViewBag.ResultCount = matchedProducts.Count;
            ViewBag.CategoryName = $"Kết quả tìm kiếm cho: {flowerName}";
